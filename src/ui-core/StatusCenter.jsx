...
export default function StatusCenter() {
  ...
  return (
    <div className="status-center">
      <button className={`status-summary status-${state}`} type="button" aria-haspopup="dialog" aria-expanded={open} onClick={() => setOpen((value) => !value)}>
      ...
      {open && (
        <section className="status-popover" role="dialog" aria-label="Runtime status">
          <header>
            <div><strong>Runtime status</strong><span>{runtime.workspaceLabel}</span></div>
            <button className="icon-button" type="button" aria-label="Close runtime status" onClick={() => setOpen(false)}><X size={15} /></button>
          </header>
          <div className="status-list">
            {items.map((entry) => (
              <div className="status-row" key={entry.id}>
                <span className={`status-dot status-dot-${tone(entry.status)}`} aria-hidden="true" />
                <div><strong>{entry.label}</strong><small>{entry.detail || entry.status}</small></div>
                <span className={`status-state status-state-${tone(entry.status)}`}>{entry.status}</span>
              </div>
            ))}
          </div>
          {state === "danger" && <footer><CircleAlert size={14} /> Missing services disable only dependent capabilities.</footer>}
        </section>
      )}
    </div>
  );
