...
export default function MobileGestureMenu({ open, onOpenChange }) {
  ...
  return (
    <div className={open ? "mobile-gesture-root open" : "mobile-gesture-root"}>
      <button className="mobile-gesture-launcher" type="button" aria-label="Open navigation" aria-expanded={open} onClick={() => onOpenChange(true)} onPointerDown={beginGesture} onPointerMove={moveLauncher} onPointerUp={endGesture} onPointerCancel={endGesture}><span aria-hidden="true" /></button>
      {open && (
        <div className="mobile-gesture-backdrop" onPointerDown={(event) => event.target === event.currentTarget && onOpenChange(false)}>
          <section className="mobile-gesture-sheet" role="dialog" aria-modal="true" aria-label="Navigation" onPointerDown={beginGesture} onPointerMove={moveSheet} onPointerUp={endGesture} onPointerCancel={endGesture}>
            <header><span className="mobile-gesture-grip" aria-hidden="true" /><div className="mobile-gesture-heading"><span className="mobile-gesture-brand-mark" aria-hidden="true">✦</span><span><strong>Quasar</strong><small>{runtime.workspaceLabel}</small></span></div><button className="icon-button" type="button" aria-label="Close navigation" onClick={() => onOpenChange(false)}><X size={18} /></button></header>
